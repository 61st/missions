...
if (EGVAR(patches,usesACRE)) exitWith {
    private _lrChannelList = parseSimpleArray EGVAR(Settings,LRsetRadioChannelNames);
    ...
    player createDiaryRecord ["61st", ["Radio", format ["<font size=20>%1</font><br/>%2", "Long Range Radio", call (player getVariable QEGVAR(document,longrange_channels))]], taskNull, "", false];
    player createDiaryRecord ["61st", ["Radio", format ["<font size=20>%1</font><br/>%2", "Radio Channels", call (player getVariable QEGVAR(document,shortrange_channels))]], taskNull, "", false];

    if (EGVAR(Settings,enableRadios)) then {
        player createDiaryRecord ["61st", ["Radio", format ["<font size=20>%1</font><br/>%2", "Radio", "    <execute expression='[] call ymf_fnc_clearRadioIds'>Reprogram Radio</execute> - Use to reset your radios frequencies and channels."]], taskNull, "", false];
    };
};
...
if (EGVAR(patches,usesTFAR)) exitWith {
    ...
    player createDiaryRecord ["61st", ["Radio", format ["<font size=20>%1</font><br/>%2", "HQ and AVIATION Nets", call (player getVariable QEGVAR(document,longrange_channels))]], taskNull, "", false];
    player createDiaryRecord ["61st", ["Radio", format ["<font size=20>%1</font><br/>%2", "Radio Channels", call (player getVariable QEGVAR(document,shortrange_channels))]], taskNull, "", false];
};
