...
([_modifier] call YMF_fnc_calculateEnemySquads) params ["_infantryCount","_lightCount","_mediumCount","_heavyCount"];

private _groups = [];
for "_i" from 1 to _infantryCount do {
	_groups pushBack ([_position] call YMF_fnc_createSquad);
};

private _lightVehicles = getArray(missionConfigFile >> "Opfor_Setup" >> "opforLightVehicles");
for "_i" from 1 to _lightCount do {
	_groups pushBack ([selectRandom _lightVehicles,_position,250,true] call YMF_fnc_createVehicle);
};

private _mediumVehicles = getArray(missionConfigFile >> "Opfor_Setup" >> "opforMediumVehicles");
for "_i" from 1 to _mediumCount do {
	_groups pushBack ([selectRandom _mediumVehicles,_position,250,true] call YMF_fnc_createVehicle);
};

private _heavyVehicles = getArray(missionConfigFile >> "Opfor_Setup" >> "opforHeavyVehicles");
for "_i" from 1 to _heavyCount do {
	_groups pushBack ([selectRandom _heavyVehicles,_position,250,true] call YMF_fnc_createVehicle);
};
...
_groups pushBack ([_target,_mediumCount] call YMF_fnc_spawnAttackHelicopters);
