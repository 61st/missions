...
([] call YMF_fnc_calculateEnemySquads) params ["","_lightCount","_mediumCount"];
...
for "_i" from 1 to _lightCount do {
	_groups pushBack ([selectRandom _lightVehicles,_startPos] call YMF_fnc_createVehicle);
};

private _mediumVehicles = getArray(missionConfigFile >> "Opfor_Setup" >> "opforMediumVehicles");
for "_i" from 1 to _mediumCount do {
	_groups pushBack ([selectRandom _mediumVehicles,_startPos] call YMF_fnc_createVehicle);
};
...
for "_i" from 1 to ceil (random 4) do {
	...
	private _group = [getText(missionConfigFile >> "Opfor_Setup" >> _type),_startPos] call YMF_fnc_createVehicle;
	_groups pushBack _group;
	private _truck = objectParent (leader _group);
	_objectives pushBack _truck;
	[_truck,_type] remoteExecCall ["YMF_fnc_addDepotRaidAction",0,_truck];
};
