...
for "_i" from 1 to _infantryCount do {
	_groups pushBack ([_position,_spawnRadius] call YMF_fnc_createSquad);
};

private _lightVehicles = getArray(missionConfigFile >> "Opfor_Setup" >> "opforLightVehicles");
for "_i" from 1 to _lightCount do {
	_groups pushBack ([selectRandom _lightVehicles,_position,_spawnRadius] call YMF_fnc_createVehicle);
};

private _mediumVehicles = getArray(missionConfigFile >> "Opfor_Setup" >> "opforMediumVehicles");
for "_i" from 1 to _mediumCount do {
	_groups pushBack ([selectRandom _mediumVehicles,_position,_spawnRadius] call YMF_fnc_createVehicle);
};

private _heavyVehicles = getArray(missionConfigFile >> "Opfor_Setup" >> "opforHeavyVehicles");
for "_i" from 1 to _heavyCount do {
	_groups pushBack ([selectRandom _heavyVehicles,_position,_spawnRadius] call YMF_fnc_createVehicle);
};

[_groups,_position,_spawnRadius] call YMF_fnc_initPatrol;
