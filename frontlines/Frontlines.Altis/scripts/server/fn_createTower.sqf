...
private _isTowerDestroyed = _sectorObj getVariable ["YMF_towerDestroyed",false];
...

{
	...
	if (_class isEqualTo "Land_Laptop_unfolded_F") then {
		[_object,_sectorObj] remoteExecCall ["YMF_fnc_gatherIntel",0,_object];
	} else {
		if (_class in ["Land_Communication_F","Land_TTowerSmall_2_F","Land_TTowerBig_1_F"]) then {
			_object addEventHandler ["Killed",YMF_fnc_towerDestroyed];
		};
	};
} forEach _objects;