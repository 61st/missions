...
_civilianCount = (_civilianCount - (_townObj getVariable ["YMF_sectorCivsKilled",0])) max 2;
private _civTypes = getArray(missionConfigFile >> "Civilian_Setup" >> "civilianTypes");
if ([] call YMF_fnc_isNight) then {
...
for "_i" from 1 to _civilianCount do {
	...
	[_group,_spawnPosition,_radius] call YMF_fnc_civilianWalking;

	if (_reputation < -50 && {25 > random 100}) then {
		[_civilian] call YMF_fnc_setupSuicideBomber;
	} else {
		if (_reputation < 0 && {abs _reputation > random 100}) then {
			...
			_civilian addEventHandler ["Hit",YMF_fnc_civilianHit];
