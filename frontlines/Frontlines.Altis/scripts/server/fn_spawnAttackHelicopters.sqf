...
private _airports = YMF_airportLocations select {[_x select 0,2000] call YMF_fnc_areaIsClear};
private _data = if (_airports isEqualTo []) then {
	[250] call YMF_fnc_findAirSpawn;
} else {
...
for "_i" from 1 to _count do {
	...
	if (_pilotClass isEqualTo "") then {
		...
		for "_i" from 1 to _crewCount do {
			private _crewMember = [_group,east,_pilotClass,_helicopter] call YMF_fnc_createUnit;
