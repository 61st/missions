...
private _militarySectors = ([["military","tower"]] call YMF_fnc_getSectorsByType) select {!(_x getVariable ["YMF_sectorOwned",false])};
private _spawnPos = [selectRandom _militarySectors,0,750,25,0,0.25,0] call BIS_fnc_findSafePos;

[format["The enemy has setup an artillery site at %1.",mapGridPosition _spawnPos],"generalNotif","Destroy Artillery"] remoteExecCall ["YMF_fnc_notify",0];
...

{
	...
	if (_class isEqualTo "PortableHelipadLight_01_yellow_F") then {
		private _group = [_artyClass,_pos,0] call YMF_fnc_createVehicle;
		_squads pushBack _group;
		if EGVAR(patches,useslambs)  then {
...
[_spawnPos,250] call YMF_fnc_createMines;
//may need scripted arty?

private _marker = ["destroyArtillery",_spawnPos,true,"ColorOPFOR","ELLIPSE",250,"Destroy Artillery","Grid"] call YMF_fnc_createMarker;
missionNamespace setVariable ["YMF_opforArtilleryActive",true,true];
...
[
	...
	},
	{
		...
		["The artillery site has been destroyed.","successNotif","Destroy Artillery"] remoteExecCall ["YMF_fnc_notify",0];
		missionNamespace setVariable ["YMF_opforArtilleryActive",nil,true];
