...
*/
missionNamespace setVariable ["YMF_secondaryActive","clearUXOs",true];
...
private _marker = ["clearUXOs",_pos,false,"ColorYellow","mil_objective",1.5,"UXOs"] call YMF_fnc_createMarker;
["The town is marked, head there to assist.","generalNotif","Clearing UXOs"] remoteExecCall ["YMF_fnc_notify",0];

[
	{
		params ["_mines"];
		(_mines select {!isNull _x && {mineActive _x}}) isEqualTo [] || {isNil "YMF_secondaryActive"}
	},
	{
		params ["_mines","_sector","_marker"];

		if (isNil "YMF_secondaryActive") then {
			["Mission cancelled.","failedNotif","Clearing UXOs"] remoteExecCall ["YMF_fnc_notify",0];
			{
				deleteVehicle _x;
			} forEach _mines;
		} else {
			["The town is now clear of UXOs.","successNotif","Clearing UXOs"] remoteExecCall ["YMF_fnc_notify",0];
			[_sector,30] call YMF_fnc_updateCivRep;
