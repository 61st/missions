...
*/
missionNamespace setVariable ["YMF_secondaryActive","medicalAid",true];

private _sectors = [["city","town","factory"],true] call YMF_fnc_getSectorsByType;
private _civReps = [];
{
...
private _marker = ["medicalAid",_chosenSector,false,"ColorYellow","mil_objective",1.5,"Medical Aid Requested"] call YMF_fnc_createMarker;
["The town is marked, head there to assist.","generalNotif","Medical Aid"] remoteExecCall ["YMF_fnc_notify",0];

[
	{
		params ["_pos"];
		!([_pos,2000] call YMF_fnc_areaIsClear) || {isNil "YMF_secondaryActive"}
	},
	{
		...
		if (isNil "YMF_secondaryActive") then {
			...
			["Mission cancelled.","failedNotif","Medical Aid"] remoteExecCall ["YMF_fnc_notify",0];
		} else {
			missionNamespace setVariable ["YMF_secondaryActive","medicalAidNoCancel",true];
			...
			for "_i" from 1 to _injuredCount do {
				...
				_civilian addEventHandler ["Killed",YMF_fnc_civilianKilled];
			...
			[_group,getPosATL _sector] call YMF_fnc_civilianWalking;
			...
			[
				...
				},
				{
					...
					[format["The aid mission is over, you saved %1 civilians.",_aliveCount],"generalNotif","Medical Aid"] remoteExecCall ["YMF_fnc_notify",0];

					private _repGain = 2 * (abs(getNumber (missionConfigFile >> "Settings" >> "killedCivRepLoss")));
					[_sector,(_aliveCount * _repGain)] call YMF_fnc_updateCivRep;
					deleteMarker _marker;
