...
missionNamespace setVariable ["YMF_secondaryActive","supplyDepot",true];
...

{
	...
	_object setVectorUp [0,0,1];
	_object setDir _dir;
	[_object,_type] remoteExecCall ["YMF_fnc_addDepotRaidAction",0,_object];

	_objectives pushBack _object;
} forEach _compositionObjectives;

private _squads = [_position,50,([] call YMF_fnc_calculateEnemySquads)] call YMF_fnc_createPatrols;
_squads pushBack ([_position,50] call YMF_fnc_createStatic);

private _marker = ["supplyDepot",_position,true,"ColorOPFOR","ELLIPSE",500,"Depot Raid","FDiagonal"] call YMF_fnc_createMarker;
["The area the depot is in has been marked.","generalNotif","Depot Raid"] remoteExecCall ["YMF_fnc_notify",0];

[
	{
		params ["_objectives","_position"];
		(_objectives select {alive _x && {(_x distance _position) < 100}}) isEqualTo [] || {isNil "YMF_secondaryActive"}
	},
	{
		params ["","_position","_marker","_objects","_squads"];

		if (isNil "YMF_secondaryActive") then {
			["Mission cancelled.","failedNotif","Depot Raid"] remoteExecCall ["YMF_fnc_notify",0];
		} else {
			["The raid is complete.","successNotif","Depot Raid"] remoteExecCall ["YMF_fnc_notify",0];
			...
			if !(missionNamespace getVariable ["YMF_opforHeaviesDisabled",false]) then {
				missionNamespace setVariable ["YMF_opforHeaviesDisabled",true,true];
				[
					{
						missionNamespace setVariable ["YMF_opforHeaviesDisabled",false,true];
					},
					[],
					5400
