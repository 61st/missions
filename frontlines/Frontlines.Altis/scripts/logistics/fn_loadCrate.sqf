...
private _offset = [];
{
	...
	if (alive _x) then {
		...
		if (_index isNotEqualTo -1) then {
			...
			private _currentCrateCount = count (_x getVariable ["YMF_loadedCrates",[]]);
...
if (isNull _vehicle) exitWith {["No suitable vehicle nearby."] call YMF_fnc_notify};

_crate attachTo [_vehicle,_offset];
[_crate,false] remoteExecCall ["enableRopeAttach",_crate];
private _currentCrates = _vehicle getVariable ["YMF_loadedCrates",[]];
_currentCrates pushBack _crate;
_vehicle setVariable ["YMF_loadedCrates",_currentCrates,true];