...
private _supportCooldowns = missionNamespace getVariable ["YMF_supportCooldowns",[]];

(parseSimpleArray (_tree tvData [_firstIndex])) params ["_action","_civRepRequired","_cooldown"];
if (CBA_missionTime < (_supportCooldowns select _firstIndex)) exitWith {["Cooldown not finished."] call YMF_fnc_notify};
if (YMF_globalReputation < _civRepRequired) exitWith {["Too low civilian reputation."] call YMF_fnc_notify};

private _pos = markerPos "supportTarget";
if (_action in ["mortar","supplyDrop"] && {_pos isEqualTo [0,0,0]}) exitWith {["No suitable target has been selected."] call YMF_fnc_notify};
...
if (_action isEqualTo "mortar") then {
	...
	["Rounds are being fired as ordered, approximately 30 seconds out."] call YMF_fnc_notify;
} else {
	if (_action isEqualTo "supplyDrop") then {
		[_pos] call YMF_fnc_createSupplyCrate;
	} else {
		if (_action isEqualTo "crateRetrieve") then {
			private _sector = objectFromNetId (_tree tvData _selectionPath);
			[_sector] remoteExecCall ["YMF_fnc_autoCrateRetrieve",2];
			[format["Guerillas are retrieving crates from %1.",_sector getVariable "YMF_sectorName"]] call YMF_fnc_notify;
		} else {
			private _sector = objectFromNetId (_tree tvData _selectionPath);
			[_sector,_action] remoteExecCall ["YMF_fnc_spawnGuerillas",2];
		};
	};
};

_supportCooldowns set [_firstIndex,(CBA_missionTime + _cooldown)];
missionNamespace setVariable ["YMF_supportCooldowns",_supportCooldowns,true];
