...
private _intelColour = if (YMF_intelCount >= _intelRequired) then {"#00BF00"} else {"#BF0000"};
...
if (_firstIndex isEqualTo 0 && {missionNamespace getVariable ["YMF_secondaryActive",""] isNotEqualTo ""}) exitWith {
	_button ctrlEnable false;
...
_button ctrlEnable ((YMF_intelCount >= _intelRequired) && {[_conditions] call YMF_fnc_conditionChecker});