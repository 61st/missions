...
private _display = createDialog ["YMF_intelMenu",true];
private _tree = _display displayCtrl 1500;
private _repLoss = getNumber (missionConfigFile >> "Settings" >> "killedCivRepLoss");
private _missions = [
	["FOB Hunt",25,"","[] remoteExecCall [""YMF_fnc_fobHunt"",2]","We have recieved some intel that a FOB is located inside the red zone on your map.<br/>
...
<t color='#00FF00'>Reward: -40 Opfor Threat</t>"],
	["Raid Enemy Depot",15,"","[] remoteExecCall [""YMF_fnc_raidSupplyDepot"",2]","We have recieved some intel that a supply depot has been setup behind enemy lines.<br/>
...
<t color='#00FF00'>Reward: +400 Resources per truck, -5 Opfor Threat, Opfor Heavy Vehicles Temporarily Disabled</t>"],
	["Secure Downed UAV",10,"","[] remoteExecCall [""YMF_fnc_captureUAV"",2]","We have recieved some intel that an enemy UAV has crashed inside the red zone on your map.<br/>
...
<t color='#00FF00'>Reward: +30 Intel<br/></t>"],
	["Setup Listening Post",0,"([['town','city','factory','military']] call YMF_fnc_getSectorsByType) isNotEqualTo []","[""init""] remoteExecCall [""YMF_fnc_listeningPost"",2]","We are able to setup a Listening Post nearby to our frontlines to gain more intel.<br/>
...
<t color='#ffa000'>Failure: If destroyed by enemy, +5 Opfor Threat</t>"],
	["Clear UXOs",3,"([['town','city','factory'],true] call YMF_fnc_getSectorsByType) isNotEqualTo []","[] remoteExecCall [""YMF_fnc_clearUXOs"",2]","The local population has requested our help to clear an area of UXOs that were left behind.<br/>
...
<t color='#00FF00'>Reward: +30 Civilian Reputation in the sector</t>"],
	["Humanitarian Relief",0,"([['town','city','factory'],true] call YMF_fnc_getSectorsByType) isNotEqualTo []","[""init""] remoteExecCall [""YMF_fnc_humanitarianRelief"",2]","We have recieved a request from a local village elder for some Humanitarian Aid.<br/>
...
<t color='#00FF00'>Reward: +20 Civilian Reputation per truck in the sector</t>"],
	["Provide Medical Aid",0,"([['town','city','factory'],true] call YMF_fnc_getSectorsByType) isNotEqualTo []","[] remoteExecCall [""YMF_fnc_provideAid"",2]",format["A local town has been hit hard by the conflict and has taken casualties.<br/>
...
private _intelActions = [
	["Mine Scanner",ceil ((count allMines) / 15),"","[] call YMF_fnc_mineScanner","Utilise some of our intelligence to detect all mines within 1000m of any player.<br/>The intel cost of this action increases with the number of mines placed."],
	["Opfor Scanner",ceil ((count (groups east)) / 2),"","[] remoteExecCall ['YMF_fnc_opforScanner',2]","Utilise some of our intelligence to detect all opfor groups and mark them on the map for 5 minutes.<br/>The intel cost of this action increases with the number of opfor units alive."]
];
...
private _titles = ["Missions","Intelligence"];
{
	private _firstIndex = _tree tvAdd [[],_titles select _forEachIndex];
	{
		...
		if (YMF_intelCount >= _intelRequired && {[_conditions] call YMF_fnc_conditionChecker}) then {
			_colour = [0,1,0,1];
		};
