...
private _civRepColour = if (YMF_globalReputation >= _civRepRequired) then {"#00BF00"} else {"#BF0000"};
...
_button ctrlEnable ((CBA_missionTime > (_supportCooldowns select _firstIndex)) && {YMF_globalReputation >= _civRepRequired});
...
if (_action isEqualTo "supplyDrop") then {
	...
	private _nearbyResources = [_pos] call YMF_fnc_getCurrentResources;
	if !([_nearbyResources,_resources] call YMF_fnc_compareResources) then {
		_button ctrlEnable false;
	};
};
