...
if (_number isNotEqualTo str(_parsed)) exitWith {["Invalid number."] call YMF_fnc_notify; false};
if (_lowest isNotEqualTo -5) exitWith {
	if (_parsed > _lowest) exitWith {true};
	[format["You need to enter a number greater then %1.",_lowest]] call YMF_fnc_notify;
	false
};
true