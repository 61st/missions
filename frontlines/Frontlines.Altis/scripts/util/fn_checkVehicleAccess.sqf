...
if (player call admp_fnc_isAdmin) exitWith {true};
...
if (_vehicle isKindOf "Air" && {_isPilot}) exitWith {
	if (player getVariable ["YMF_role","rifleman"] isEqualTo "pilot") exitWith {
		true;
	};
	["You must be a pilot to use this vehicle."] call YMF_fnc_notify;
	moveOut player;
...
if (_vehicle isKindOf "Tank" && {!(_vehicle isKindOf "APC_Tracked_02_base_F")}) exitWith {
	if (player getVariable ["YMF_role","rifleman"] in ["commander","crewman","engineer"]) exitWith {
		true;
	};
	["You are not authorised use this vehicle."] call YMF_fnc_notify;
	moveOut player;
	if !(_moveOutOnly) then {player moveInCargo _vehicle};
	false;
};
