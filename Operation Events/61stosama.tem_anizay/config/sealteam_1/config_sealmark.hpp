class sealteammark1 {
    name = "Operator Marksman";
    description = "Its a grunts life";
    traits[] = {};
    customVariables[] = {
        {"ace_medical_medicClass",0,true},
        {"ace_isEngineer",1,true},
        {"YMF_iscom", true, true}
    };
    icon = "z\LXIM\addons\media\images\Icons\iconman_red.paa";

    defaultLoadout[] = {
        {"rhs_weap_m14ebrri","muzzle_snds_B","rhsusf_acc_anpeq15side_bk","rhsusf_acc_ACOG_RMR",{"rhsusf_20Rnd_762x51_m993_Mag",20},{},"rhsusf_acc_harris_bipod"},{},
        {"rhsusf_weap_glock17g4","rhsusf_acc_omega9k","acc_flashlight_pistol","",{"rhsusf_mag_17Rnd_9x19_JHP",17},{},""},
        {"rhs_uniform_FROG01_wd",{{"lxim_medbags_FirstAid",1}}},
        {"rhsusf_plateframe_teamleader",{{"rhsusf_mag_17Rnd_9x19_JHP",2,17},{"rhs_mag_mk84",1,1},{"ACE_30Rnd_556x45_Stanag_M995_AP_mag",14,30}}},
        {"lxim_backpack_Kitbag_Multicam",{{"ACE_EntrenchingTool",1},{"ItemcTabMisc",1},{"ChemicalDetector_01_black_F",1},{"ACE_DefusalKit",1},{"tsp_lockpick",1},{"ACE_Clacker",1},{"ACE_Flashlight_XL50",1},{"rhs_mag_m67",2,1},{"rhs_grenade_anm8_mag",2,1},{"ACE_M84",2,1},{"MS_Strobe_Mag_1",2,1},{"DemoCharge_Remote_Mag",2,1}}},
        "H_HelmetB_light_snakeskin","G_Bandanna_Syndikat2",{},
        {"ItemMap","ItemMicroDAGR","TFAR_anprc152","ItemCompass","ACE_Altimeter","A3_GPNVG18b_TI"}
    };
    // for 1-1
    arsenalitems[] = {
        "1Rnd_HE_Grenade_shell",
        "ACE_HuntIR_M203",
        "ACE_LAMG_SpareBarrel",
        "ACE_LWAMG_SpareBarrel",
        "launch_MRAWS_green_F",
        "launch_MRAWS_olive_F",
        "launch_MRAWS_sand_F",
        "lxim_sig_M250",
        "lxim_sig_M250_BLK",
        "rhs_mag_M397_HET",
        "rhs_mag_m4009",
        "rhs_mag_M433_HEDP",
        "rhs_mag_M441_HE",
        "rhs_mag_m576",
        "rhs_mag_M583A1_white",
        "rhs_mag_M585_white_cluster",
        "rhs_mag_m661_green",
        "rhs_mag_m662_red",
        "rhs_mag_M663_green_cluster",
        "rhs_mag_M664_red_cluster",
        "rhs_mag_m713_Red",
        "rhs_mag_m714_White",
        "rhs_mag_m715_Green",
        "rhs_mag_m716_yellow",
        "rhs_weap_fgm148",
        "rhs_weap_fim92",
        "rhs_weap_m240B",
        "rhs_weap_m240G",
        "rhs_weap_M320",
        "rhsusf_acc_ARDEC_M240",
        "UGL_FlareGreen_F",
        "rhs_weap_hk416d145_wd",
        "rhs_weap_hk416d145_wd_2",
        "arifle_SPAR_01_blk_F",
        "arifle_SPAR_01_GL_blk_F",
        "rhs_weap_hk416d145_m320",
        "rhs_weap_hk416d145",
        "rhs_weap_hk416d10_LMT_wd",
        "rhs_weap_hk416d10_LMT_d",
        "rhs_weap_hk416d10_LMT",
        "rhs_weap_hk416d10_m320",
        "rhs_weap_hk416d10",
        "rhs_uniform_FROG01_wd",
        "rhs_uniform_FROG01_d",
        "H_HelmetHBK_headset_F",
        "H_HelmetHBK_F",
        "H_Booniehat_mgrn",
        "H_Booniehat_khk_hs",
        "H_Booniehat_khk",
        "H_Booniehat_mcamo",
        "H_Booniehat_oli",
        "H_Booniehat_tan",
        "H_Booniehat_wdl",
        "rhs_booniehat2_marpatd",
        "rhs_booniehat2_marpatwd",
        "rhsusf_opscore_aor1",
        "rhsusf_opscore_aor1_pelt",
        "rhsusf_opscore_aor1_pelt_nsw",
        "rhsusf_opscore_aor2",
        "rhsusf_opscore_aor2_pelt",
        "rhsusf_opscore_aor2_pelt_nsw",
        "rhsusf_opscore_bk",
        "rhsusf_opscore_bk_pelt",
        "rhsusf_opscore_coy_cover",
        "rhsusf_opscore_fg",
        "rhsusf_opscore_fg_pelt",
        "rhsusf_opscore_coy_cover_pelt",
        "rhsusf_opscore_fg_pelt_cam",
        "rhsusf_opscore_fg_pelt_nsw",
        "rhsusf_opscore_mc_cover",
        "rhsusf_opscore_mc_cover_pelt",
        "rhsusf_opscore_mc_cover_pelt_nsw",
        "rhsusf_opscore_mc_cover_pelt_cam",
        "rhsusf_opscore_mc",
        "rhsusf_opscore_mc_pelt",
        "rhsusf_opscore_mc_pelt_nsw",
        "rhsusf_opscore_paint",
        "rhsusf_opscore_paint_pelt",
        "rhsusf_opscore_paint_pelt_nsw",
        "rhsusf_opscore_paint_pelt_nsw_cam",
        "rhsusf_opscore_rg_cover",
        "rhsusf_opscore_rg_cover_pelt",
        "rhsusf_opscore_ut",
        "rhsusf_opscore_ut_pelt",
        "rhsusf_opscore_ut_pelt_cam",
        "rhsusf_opscore_ut_pelt_nsw",
        "rhsusf_opscore_ut_pelt_nsw_cam",
        "rhsusf_opscore_mar_fg",
        "rhsusf_opscore_mar_fg_pelt",
        "rhsusf_opscore_mar_ut",
        "rhsusf_opscore_mar_ut_pelt",
        "H_HelmetSpecB_snakeskin",
        "H_HelmetB_light_snakeskin",
        "V_CarrierRigKBT_01_light_EAF_F",
        "ItemcTab",
        "G_Balaclava_TI_blk_F",
        "G_Squares_Tinted",
        "G_Squares",
        "G_Sport_Greenblack",
        "G_Sport_Blackred",
        "G_Sport_Checkered",
        "G_Sport_BlackWhite",
        "G_Sport_Blackyellow",
        "G_Sport_Red",
        "rhsusf_shemagh2_gogg_tan",
        "rhsusf_shemagh_tan",
        "rhsusf_shemagh2_tan",
        "rhsusf_shemagh2_od",
        "rhsusf_shemagh_od",
        "rhsusf_shemagh_grn",
        "rhsusf_shemagh2_grn",
        "G_LEN_TG1_blauw",
        "G_tweed_tacticool_blauw",
        "G_Bandanna_Syndikat2",
        "G_Bandanna_Vampire_01",
        "G_Bandanna_tan",
        "G_Bandanna_sport",
        "G_Bandanna_Skull2",
        "G_Bandanna_Syndikat1",
        "G_Bandanna_Skull1",
        "G_Bandanna_shades",
        "G_Bandanna_oli",
        "G_Bandanna_beast",
        "G_Bandanna_aviator",
        "G_Bandanna_blk",
        "rhs_googles_black",
        "G_Balaclava_Skull1",
        "G_Balaclava_Scarecrow_01",
        "G_Balaclava_lowprofile",
        "psq42_blk",
        "psq42_blk_icup",
        "psq42_blk_lenscap",
        "psq42_blk_mount",
        "TFAR_anprc152",
        "ACE_DefusalKit",
        "ItemcTabMisc",
        "rhs_mag_30Rnd_556x45_Mk318_Stanag",
        "rhs_mag_30Rnd_556x45_M855_PMAG",
        "UGL_FlareGreen_F",
        "UGL_FlareYellow_F",
        "1Rnd_SmokeBlue_Grenade_shell",
        "UGL_FlareWhite_F",
        "UGL_FlareCIR_F",
        "rhs_mag_M583A1_white",
        "rhs_mag_30Rnd_556x45_Mk318_PMAG_Tan",
        "rhs_mag_30Rnd_556x45_M193_Stanag",
        "rhs_mag_M433_HEDP",
        "ACE_40mm_Flare_red",
        "rhs_mag_30Rnd_556x45_Mk262_Stanag_Ranger",
        "ACE_40mm_Flare_white",
        "rhs_mag_30Rnd_556x45_M855_PMAG_Tan",
        "rhs_mag_M397_HET",
        "rhs_mag_m662_red",
        "rhs_mag_M663_green_cluster",
        "rhs_mag_30Rnd_556x45_M855A1_Stanag_Ranger",
        "1Rnd_SmokeYellow_Grenade_shell",
        "rhs_mag_30Rnd_556x45_M855_Stanag",
        "1Rnd_Smoke_Grenade_shell",
        "1Rnd_SmokePurple_Grenade_shell",
        "1Rnd_SmokeOrange_Grenade_shell",
        "rhs_mag_m714_White",
        "rhs_mag_30Rnd_556x45_M855_Stanag_Ranger",
        "rhs_mag_30Rnd_556x45_Mk318_Stanag_Pull",
        "rhs_mag_30Rnd_556x45_Mk318_PMAG",
        "rhs_mag_M585_white_cluster",
        "rhs_mag_m715_Green",
        "rhs_mag_30Rnd_556x45_M855A1_PMAG_Tan",
        "rhs_mag_M664_red_cluster",
        "rhs_mag_m713_Red",
        "rhs_mag_30Rnd_556x45_M855A1_Stanag",
        "1Rnd_HE_Grenade_shell",
        "rhs_mag_m661_green",
        "rhs_mag_30Rnd_556x45_Mk262_Stanag",
        "rhs_mag_30Rnd_556x45_M855A1_Stanag_Pull",
        "rhs_mag_30Rnd_556x45_Mk318_Stanag_Ranger",
        "ACE_40mm_Flare_green",
        "rhs_mag_30Rnd_556x45_M855A1_PMAG",
        "ACE_HuntIR_M203",
        "rhs_mag_M441_HE",
        "rhs_mag_m716_yellow",
        "rhs_mag_30Rnd_556x45_Mk262_PMAG",
        "rhs_mag_30Rnd_556x45_Mk262_PMAG_Tan",
        "UGL_FlareRed_F",
        "1Rnd_SmokeRed_Grenade_shell",
        "1Rnd_SmokeGreen_Grenade_shell",
        "rhsusf_weap_glock17g4",
        "rhsusf_mag_17Rnd_9x19_JHP",
        "rhsusf_acc_anpeq15_top",
        "rhsusf_acc_anpeq15side",
        "rhsusf_acc_anpeq16a",
        "rhsusf_acc_anpeq16a_top",
        "rhs_acc_perst1ik_ris",
        "rhsusf_acc_anpeq15_wmx",
        "rhsusf_acc_wmx",
        "rhsusf_acc_anpeq15A",
        "MHS_X300U",
        "acc_flashlight",
        "ACE_DBAL_A3_Green",
        "rhsusf_acc_anpeq15side_bk",
        "rhs_acc_2dpZenit_ris",
        "rhsusf_acc_anpeq15",
        "rhsusf_acc_anpeq15_bk_top",
        "rhsusf_acc_wmx_bk",
        "ACE_DBAL_A3_Red",
        "rhsusf_acc_anpeq15_bk",
        "ACE_SPIR",
        "rhsusf_acc_eotech_xps3",
        "rhsusf_acc_ELCAN_ard",
        "optic_tws_mg",
        "optic_MRCO",
        "optic_Yorris",
        "lxim_equipment_optic_Nightstalker",
        "lxim_equipment_optic_AMSTI_snd",
        "rhsusf_acc_T1_low_fwd",
        "rhs_acc_dh520x56",
        "rhs_acc_ekp8_18",
        "rhsusf_acc_ACOG2_USMC",
        "optic_tws",
        "optic_DMS_weathered_Kir_F",
        "lxim_equipment_optic_AMS_snd",
        "rhsusf_acc_eotech_552_wd",
        "rhsusf_acc_su230_c",
        "rhsusf_acc_anpas13gv1",
        "optic_Holosight_khk_F",
        "optic_ERCO_snd_F",
        "lxim_equipment_optic_AMS",
        "lxim_equipment_optic_AMSTI_khk",
        "rhsusf_acc_g33_xps3_tan",
        "rhsusf_acc_su230a_mrds_c",
        "optic_ACO_grn",
        "optic_DMS_weathered_F",
        "lxim_equipment_xm157_prototype",
        "rhsusf_acc_ACOG2",
        "ACE_optic_MRCO_2D",
        "rhsgref_acc_RX01_camo",
        "rhsusf_acc_ACOG3",
        "rhsusf_acc_ACOG_d",
        "optic_Aco",
        "optic_Holosight_arid_F",
        "optic_Holosight_smg",
        "rhsusf_acc_RX01_NoFilter_tan",
        "rhsusf_acc_ACOG_USMC",
        "rhsusf_acc_ACOG3_USMC",
        "rhsusf_acc_su230a_mrds",
        "ACE_optic_Hamr_2D",
        "rhsusf_acc_EOTECH",
        "rhsusf_acc_RX01",
        "rhsusf_acc_ACOG",
        "rhsusf_acc_ELCAN",
        "rhsusf_acc_ACOG_MDO",
        "optic_Hamr",
        "optic_Holosight_smg_khk_F",
        "optic_ERCO_khk_F",
        "lxim_equipment_optic_DMS",
        "rhsusf_acc_g33_xps3",
        "rhsusf_acc_compm4",
        "optic_ACO_grn_smg",
        "lxim_equipment_optic_AMSTI",
        "rhsusf_acc_T1_low",
        "rhsusf_acc_su230a",
        "rhsusf_acc_su230a_c",
        "rhsusf_acc_anpvs27",
        "rhsusf_acc_ACOG_RMR",
        "rhsusf_acc_su230",
        "rhsusf_acc_su230_mrds",
        "optic_ERCO_blk_F",
        "rhsusf_acc_eotech_552",
        "rhsusf_acc_g33_T1",
        "rhsusf_acc_ACOG_wd",
        "optic_Holosight_blk_F",
        "optic_Holosight_lush_F",
        "rhsgref_acc_RX01_NoFilter_camo",
        "lxim_equipment_optic_AMS_khk",
        "rhsusf_acc_T1_high",
        "rhsusf_acc_RX01_NoFilter",
        "optic_DMS",
        "optic_DMS_ghex_F",
        "rhsusf_acc_eotech_552_d",
        "rhsusf_acc_RX01_tan",
        "rhsusf_acc_su230_mrds_c",
        "optic_Aco_smg",
        "optic_Hamr_khk_F",
        "optic_Holosight",
        "optic_Holosight_smg_blk_F",
        "optic_KHS_blk",
        "rhsusf_acc_rotex5_grey",
        "muzzle_snds_m_snd_F",
        "rhsusf_acc_SF3P556",
        "ACE_muzzle_mzls_L",
        "rhsusf_acc_SFMB556",
        "muzzle_snds_M",
        "muzzle_snds_m_khk_F",
        "ACE_muzzle_mzls_smg_02",
        "rhsusf_acc_rotex5_tan",
        "rhsusf_acc_omega9k",
        "bipod_02_F_blk",
        "rhsusf_acc_grip1",
        "rhsusf_acc_grip2_tan",
        "bipod_01_F_khk",
        "rhs_acc_grip_ffg2",
        "rhsusf_acc_rvg_blk",
        "rhsusf_acc_rvg_de",
        "bipod_01_F_blk",
        "bipod_02_F_lush",
        "rhsusf_acc_grip2_wd",
        "bipod_01_F_mtp",
        "bipod_02_F_hex",
        "bipod_03_F_blk",
        "bipod_02_F_arid",
        "rhs_acc_harris_swivel",
        "rhsusf_acc_tdstubby_blk",
        "rhsusf_acc_tdstubby_tan",
        "rhsusf_acc_harris_bipod",
        "bipod_01_F_snd",
        "bipod_03_F_oli",
        "rhsusf_acc_kac_grip",
        "rhsusf_acc_grip2",
        "bipod_02_F_tan",
        "rhsusf_acc_grip3",
        "rhsusf_acc_grip3_tan",
        "rhsusf_mag_17Rnd_9x19_FMJ",
        "rhs_mag_30Rnd_556x45_M855A1_EPM_Ranger",
        "rhs_mag_30Rnd_556x45_M855A1_EPM",
        "rhs_mag_30Rnd_556x45_M855_Stanag_Pull",
        "rhs_mag_30Rnd_556x45_Mk262_Stanag_Pull",
        "optic_Arco_blk_F",
        "optic_Arco_AK_blk_F",
        "optic_Arco_AK_arid_F",
        "acc_flashlight_pistol",
        "MHS_M17",
        "MHS_M18",
        "rhs_weap_M136",
        "rhs_weap_M136_hedp",
        "rhs_weap_M136_hp",
        "rhs_weap_m72a7",
        "psq42_blk",
        "psq42_blk_icup",
        "psq42_blk_lenscap",
        "psq42_od3",
        "psq42_od3_icup",
        "psq42_od3_lenscap",
        "B_simc_US_Molle_sturm_OCP",
        "lxim_backpack_TacticalPack_Multicam_Woodland",
        "rhs_weap_m249_pip_S",
        "rhsusf_200rnd_556x45_M855_box",
        "rhsusf_200Rnd_556x45_M855_soft_pouch",
        "rhsusf_200Rnd_556x45_M855_soft_pouch_ucp",
        "rhsusf_200Rnd_556x45_soft_pouch_ucp",
        "rhsusf_200Rnd_556x45_M995_soft_pouch_coyote",
        "rhsusf_200Rnd_556x45_box",
        "rhsusf_200Rnd_556x45_soft_pouch_coyote",
        "KAR_100Rnd_Fury_blk","KAR_100Rnd_Fury",
        "rhsusf_100Rnd_556x45_M855_soft_pouch",
        "rhsusf_100Rnd_556x45_M855_soft_pouch_ucp",
        "rhsusf_100Rnd_556x45_soft_pouch_coyote",
        "rhsusf_100Rnd_556x45_soft_pouch",
        "rhsusf_100Rnd_556x45_M995_soft_pouch",
        "rhsusf_100Rnd_556x45_M995_soft_pouch_ucp",
        "rhsusf_200Rnd_556x45_M995_soft_pouch",
        "lxim_sig_150Rnd_Fury",
        "lxim_sig_150Rnd_Fury_blk",
        "lxim_sig_M250","lxim_sig_M250_BLK",
        "rhs_weap_m249_pip_S_para",
        "lxim_backpack_B_RadioBag_01_Solid_Black_F",
        "B_simc_US_Molle_sturm_OCP_RTO",
        "B_simc_US_Molle_sturm_OCP_RTO_wasser",
        "rhs_weap_sr25_ec",
        "rhsusf_20Rnd_762x51_SR25_m993_Mag",
        "rhs_weap_m14ebrri",
        "rhs_weap_XM2010",
        "rhs_weap_m24sws",
        "rhs_weap_m40a5",
        "ACE_20Rnd_762x51_M993_AP_Mag",
        "rhsusf_5Rnd_300winmag_xm2010",
        "rhsusf_10Rnd_762x51_m993_Mag",
        "rhsusf_20Rnd_762x51_m993_Mag",
        "rhsusf_5Rnd_762x51_m993_Mag",
        "lxim_equipment_optic_Nightstalker",
        "lxim_equipment_optic_AMSTI_snd",
        "rhsusf_acc_LEUPOLDMK4_2_mrds",
        "rhsusf_acc_LEUPOLDMK4_wd",
        "rhsusf_acc_nxs_3515x50f1_h58",
        "rhsusf_acc_ACOG2_USMC",
        "lxim_equipment_optic_AMS_snd",
        "rhsusf_acc_LEUPOLDMK4_2_d",
        "rhsusf_acc_premier",
        "rhsusf_acc_M8541_wd",
        "rhsusf_acc_anpas13gv1",
        "lxim_equipment_optic_AMS",
        "lxim_equipment_optic_AMSTI_khk",
        "rhsusf_acc_M8541_low_wd",
        "rhsusf_acc_nxs_3515x50f1_md_sun",
        "lxim_equipment_xm157_prototype",
        "rhsusf_acc_nxs_5522x56_md_sun",
        "rhsusf_acc_premier_mrds",
        "rhsusf_acc_LEUPOLDMK4_d",
        "rhsusf_acc_premier_low",
        "rhsusf_acc_RX01_NoFilter_tan",
        "rhsusf_acc_ACOG_USMC",
        "rhsusf_acc_ACOG3_USMC",
        "lxim_equipment_optic_DMS",
        "rhsusf_acc_nxs_3515x50f1_md",
        "lxim_equipment_optic_AMSTI",
        "rhsusf_acc_LEUPOLDMK4",
        "rhsusf_acc_M8541_d",
        "rhsusf_acc_M8541_low",
        "rhsusf_acc_nxs_3515x50f1_h58_sun",
        "rhsusf_acc_anpvs27",
        "rhsusf_acc_premier_anpvs27",
        "rhsusf_acc_LEUPOLDMK4_2",
        "rhsusf_acc_nxs_5522x56_md",
        "rhsusf_acc_ACOG_RMR",
        "lxim_equipment_optic_AMS_khk",
        "rhsusf_acc_M8541_mrds",
        "rhsusf_acc_T1_high",
        "rhsusf_acc_M8541",
        "rhsusf_acc_M8541_low_d",
        "rhsusf_acc_nxs_3515x50_md",
        "rhsusf_5Rnd_762x51_AICS_m993_Mag",
        "rhsusf_acc_ACOG_anpvs27",
        "optic_AMS",
        "ACE_10Rnd_762x51_Mag_Tracer_Dim",
        "ACE_SpottingScope",
        "Rangefinder",
        "ACE_Vector",
        "lxim_equipment_Vector_Designator",
        "lxim_equipment_Vector_Designator_NVG",
        "rhs_fim92_mag",
        "rhs_mag_maaws_HE",
        "rhs_mag_maaws_HEDP",
        "rhs_fgm148_magazine_AT",
        "rhs_mag_maaws_HEAT",
        "A3_GPNVG18b_TI"

    };
};
