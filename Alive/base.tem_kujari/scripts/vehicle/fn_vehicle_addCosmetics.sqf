...
if (_vehicle isKindOf "lxim_vanilla_mk20") then {
...
if (_vehicle isKindOf "B_abramsx") then { 
    _vehicle animateSource ["extraantennas",1, true]; 
    _vehicle animateSource ["rubberskirts",1, true]; 
    [_vehicle, [
        [0, "z\lxim\addons\media\images\vehicles\m1x\abramsx_sand.paa"],
        [1, "A3\Armor_F\Data\camonet_NATO_Desert_CO.paa"]
    ]] call EFUNC(vehicle,applyTextures);
};
...
if (_vehicle isKindOf "rhsusf_m1151_base") then {
    _vehicle setVariable ["RHS_Decal_Enabled", false, true];
...
if (_vehicle isKindOf "rhsusf_m1152_base") then { 
    _vehicle setVariable ["RHS_Decal_Enabled", false, true];
