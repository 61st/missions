...
private _groupToUpdate = SIXONE_dynamicGroups select _groupIndex;
...
private _oldSelectionPath = if (_isRespawn) then {[]} else {[_unit] call SIXONE_fnc_removeFromGroup};
...
[_desiredRole,_isRespawn] remoteExecCall ["SIXONE_fnc_setupPlayer",_unit];

[_selectedGroup,_unit] remoteExecCall ["selectLeader",groupOwner _selectedGroup];

[SIXONE_dynamicGroups,_oldSelectionPath,_selectionPath,_unit] remoteExecCall ["SIXONE_fnc_updateGroups",-2,"SIXONE_DG_JIP"];