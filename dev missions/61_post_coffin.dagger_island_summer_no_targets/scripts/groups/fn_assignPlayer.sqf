...
private _oldSelectionPath = if (_isRespawn) then {[]} else {[_unit] call LXIF_fnc_removeFromGroup};
...
[_desiredRole,_isRespawn] remoteExecCall ["LXIF_fnc_setupPlayer",_unit];

[_selectedGroup,_unit] remoteExecCall ["selectLeader",groupOwner _selectedGroup];

[LXIF_dynamicGroups,_oldSelectionPath,_selectionPath,_unit] remoteExecCall ["LXIF_fnc_updateGroups",-2,"LXIF_DG_JIP"];