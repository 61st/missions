 ...
 *
 * Example:
 * call Scripts_fnc_getArsenalWhitelist;
 */
...
private _orgItems = switch (_organization#1) do {
    case "Outlaw": {
        GET_CONTAINER_KEYS("Outlaw");
    };
    case "Thunder": {
        GET_CONTAINER_KEYS("Thunder");
    };
    case "Phoenix": {
        GET_CONTAINER_KEYS("Phoenix");
    };
...
private _roleSpecific = switch ([player] call EFUNC(gear,getLoadoutRole)) do {
    ...
    case "dismount": {GET_CONTAINER_KEYS("arsenal_role_dismount");};
    case "crew": {GET_CONTAINER_KEYS("arsenal_role_crew");};
    case "pilot";
