...
private _sectors = [];
{
	...
	if (_type in ["tower","factory","military","city","town"]) then {
		...
		_sectorNamespace setVariable ["YMF_sectorVariable",_x,true];
		_sectorNamespace setVariable ["YMF_sectorName",markerText _x,true];
		_sectorNamespace setVariable ["YMF_sectorType",_type,true];

		_sectors pushBack _sectorNamespace;
	};
} forEach allMapMarkers;
missionNamespace setVariable ["YMF_allSectors",_sectors,true];

YMF_airportLocations = [];
...
YMF_airportLocations pushBack [_primAirport,_primAirportDirection];

{
	...
	YMF_airportLocations pushBack [_secAirport,_secAirportDirection];
