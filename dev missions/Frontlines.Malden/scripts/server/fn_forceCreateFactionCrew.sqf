...
private _driver = [_vehGroup,_side,selectRandom _squadComposition,_emptyVehicle] call YMF_fnc_createUnit;
_driver moveInDriver _emptyVehicle;
if (fullCrew [_emptyVehicle,"gunner",true] isNotEqualTo []) then {
	private _gunner = [_vehGroup,_side,selectRandom _squadComposition,_emptyVehicle] call YMF_fnc_createUnit;
	_gunner moveInGunner _emptyVehicle;
};
if (fullCrew [_emptyVehicle,"commander",true] isNotEqualTo []) then {
	private _commander = [_vehGroup,_side,selectRandom _squadComposition,_emptyVehicle] call YMF_fnc_createUnit;
