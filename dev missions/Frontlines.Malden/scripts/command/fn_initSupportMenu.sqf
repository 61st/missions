...
private _display = createDialog ["YMF_supportMenu",true];
...

{
	...
	if (YMF_globalReputation >= _civRepRequired) then {
		_colour = [0,1,0,1];
	};
	...
	if (_action isEqualTo "mortar") then {
		...
		if (_action isEqualTo "supplyDrop") then {
			...
			if (_action isEqualTo "crateRetrieve") then {
				...
				{
					private _radius = (getArray(missionConfigFile >> "Settings" >> "sectorRadius")) select ([_x getVariable "YMF_sectorType"] call YMF_fnc_getSectorTypeIndex);
					private _crates = nearestObjects [_x,_crateTypes,(_radius * 1.1),true];
					if (_crates isNotEqualTo [] && {!(_x in YMF_activeSectors)}) then {
						private _index = _tree tvAdd [[_actionIndex],_x getVariable "YMF_sectorName"];
						_tree tvSetData [[_actionIndex,_index],netId _x];
					};
				} forEach YMF_allSectors;
			} else {
				if (_action in ["infAttack","mechAttack","sectorDefence"]) then {
					private _targetSectors = if (_action isEqualTo "sectorDefence") then {YMF_activeSectors select {_x getVariable ["YMF_sectorOwned",false]}} else {YMF_activeSectors select {!(_x getVariable ["YMF_sectorOwned",false])}};
					{
						private _index = _tree tvAdd [[_actionIndex],_x getVariable "YMF_sectorName"];
						_tree tvSetData [[_actionIndex,_index],netId _x];
