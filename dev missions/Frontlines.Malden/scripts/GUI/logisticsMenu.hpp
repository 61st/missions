class YMF_logisticsMenu : RscDisplayTeamSwitch
{
	idd=9645;
	name="YMF_logisticsMenu";
	movingEnable=0;
	onLoad = "";
	onUnload = "";
	class Controls : Controls
	{
		...
		{
			idc = 1500;
			onLbSelChanged = "_this call YMF_fnc_onLogisticsMenuLbChange;";
		...
		{
			idc = 1200;
			onMouseButtonDown = "_this call YMF_fnc_onFactoryMapPress";
		...
		{
			idc = 2401;
			onButtonClick = "[] call YMF_fnc_logisticsMenuStartRoute;";
		...
		{
			idc = 2400;
			onButtonClick = "[player] remoteExecCall ['YMF_fnc_buyNewTruck',2];";
		...
		{
			idc = 2100;
			onLBSelChanged = "_this call YMF_fnc_onLogisticsMenuComboChange";
			x = "16.3 * 					(			((safezoneW / safezoneH) min 1.2) / 40) + 		(safezoneX + (safezoneW - 					((safezoneW / safezoneH) min 1.2))/2)";
		...
		class RscEdit_1400: RscEdit
		{
			...
			onEditChanged = "[] call YMF_fnc_onLogisticsMenuComboChange";
		};
