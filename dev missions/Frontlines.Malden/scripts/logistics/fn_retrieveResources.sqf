...
if (!([_desiredSupplies,-0.1] call YMF_fnc_checkNumber) || {!([_desiredFuel,-0.1] call YMF_fnc_checkNumber) || {!([_desiredAmmo,-0.1] call YMF_fnc_checkNumber)}}) exitWith {};

private _safePosition = (getPosATL player) findEmptyPosition [0,75,"Land_HelipadEmpty_F"];
if (_safePosition isEqualTo []) exitWith {["No suitable position to spawn crates, move somewhere else."] call YMF_fnc_notify};
...
private _isAtFOB = [player] call YMF_fnc_isNearFOB;
...
if (_isAtFOB) then {
	...
	_nearestFactory = [player,["factory"]] call YMF_fnc_getNearestSector;
...
if !(_enoughResources) exitWith {["Not enough resources are stored here."] call YMF_fnc_notify};
closeDialog 0;
...
if (_isAtFOB) then {
	[player,_desiredResourceCounts,false] call YMF_fnc_adjustResources;
} else {
	_nearestFactory setVariable ["YMF_factoryResources",_storedResources vectorDiff _desiredResourceCounts,true];
};