...
private _type = _object getVariable "YMF_factoryType";
private _resources = _object getVariable "YMF_factoryResources";
if (_type isEqualTo _toolboxIndex) exitWith {["No update required."] call YMF_fnc_notify};
_resources params ["_suppliesAvailable"];
if (_suppliesAvailable < 100) exitWith {["100 supplies need to be available at the factory to change its production."] call YMF_fnc_notify};

_resources = _resources vectorDiff [100,0,0];
_object setVariable ["YMF_factoryType",_toolboxIndex,true];
_object setVariable ["YMF_factoryResources",_resources,true];

["Factory updated."] call YMF_fnc_notify;
