...
if (_action) then {
	private _mobileRespawnIndex = (missionNamespace getVariable ["YMF_mobileRespawns",[]]) find _object;
	...
	if (_object isKindOf "Air") then {
		private _heliIndex = (missionNamespace getVariable ["YMF_currentHelicopters",[]]) find _object;
		if (_heliIndex isNotEqualTo -1) then {
			YMF_currentHelicopters deleteAt _heliIndex;
			publicVariable "YMF_currentHelicopters";
		} else {
			private _planeIndex = (missionNamespace getVariable ["YMF_currentPlanes",[]]) find _object;
	...
	[_object,_resourcesToReturn,true] call YMF_fnc_adjustResources;
