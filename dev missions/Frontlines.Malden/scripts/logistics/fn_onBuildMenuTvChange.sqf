...
if (_conditions isNotEqualTo "") then {
	...
	if ("YMF_allFOBs" in _conditions) then {
	...
	if ("YMF_canPurchaseAir" in _conditions) then {
		_extraText = format ["%1A radar present at any FOB<br/>",_extraText];
	};

	if ("YMF_currentHelicopters" in _conditions) then {
		_extraText = format ["%1A spare helipad at any FOB<br/>",_extraText];
	};

	if ("YMF_maxPlaneCount" in _conditions) then {
		_extraText = format ["%1A spare hangar at any FOB<br/>",_extraText];
	};

	if ("YMF_globalReputation" in _conditions) then {
		private _startIndex = _conditions find "YMF_globalReputation";
	...
	if ("YMF_fnc_getMissionProgress" in _conditions) then {
		private _startIndex = _conditions find "YMF_fnc_getMissionProgress";
